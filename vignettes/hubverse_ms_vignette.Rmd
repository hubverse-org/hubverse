...
We can now visualise the ensemble forecast as well as individual model performance all together. 
...
As aforementioned, to evaluate model outputs we use oracle output data, a form of target data which matches and can be merged with hubverse-style model output data but which contains a point prediction representing the eventual observation, as if observed by an oracle model. 
...
```{r, evalFALSE}
target_data |>
  select(date, location, observation) |>
  rename(
